...
class VideoSwinTransformerBackbone(nn.Module):
    """
    A wrapper which allows using Video-Swin Transformer as a temporal encoder for WCL.
    ...
    Additionally, we slightly modify video-swin to make it output per-frame embeddings as required by WCL (check our
...
class Joiner(nn.Sequential):
    ...
    def forward(self, tensor_list: NestedTensor):
        t, _ = tensor_list.tensors.shape[:2]
        tensor_list.tensors = rearrange(tensor_list.tensors, 't b c h w -> (b t) c h w')
        tensor_list.mask = rearrange(tensor_list.mask, 't b h w -> (b t) h w')
